...
CGameFramework::CGameFramework()
{
	...
	for (int i = 0; i < 50; ++i) m_pszFrameRate[i] = 0;
...
{
	m_hInstance = hInstance;				// Instance 객체설정
	m_hWnd = hWnd;							// hwnd 객체설정

	CreateDirect3DDevice();					// D3D device 생성
	CreateCommandQueueAndList();			// 명령 큐 생성
	CreateRtvAndDsvDescriptorHeaps();		// Render Target and Depth Stencil Heap 생성
	CreateSwapChain();						// 화면 버퍼 생성
	CreateRenderTargetViews();				// Render Target 설정 (Desc)
	CreateDepthStencilView();				// Depth Stencil 설정 (Desc)

	BuildObjects();							// 카메라, Player, Scene객체 생성 및 초기화

	::srand(timeGetTime());
	_tcscpy_s(m_pszFrameRate, _T("LabProject ("));		// 프로그램 타이틀
	return true;
}
...
void CGameFramework::BuildObjects()
{
	...
	//m_pCamera->SetViewport(0, 0, m_nWndClientWidth, m_nWndClientHeight, 0.0f, 1.0f);		// 뷰포트
	//m_pCamera->SetScissorRect(0, 0, m_nWndClientWidth, m_nWndClientHeight);					// 씨저 사각형
	//m_pCamera->GenerateProjectionMatrix(1.0f, 500.0f, float(m_nWndClientWidth) /			// 투영 변환 행렬
	//	float(m_nWndClientHeight), 90.0f);
	//m_pCamera->GenerateViewMatrix(XMFLOAT3(0.0f, 15.0f, -25.0f),XMFLOAT3(0.0f, 0.0f, 0.0f),XMFLOAT3(0.0f, 1.0f, 0.0f));	// 카메라 변환 행렬
	...
	////pCamera->GeneratePerspectiveProjectionMatrix(1.01f, 500.0f, 60.0f);		// 뷰포트 변환..?
	////pCamera->SetFOVAngle(60.0f);											// FOV 설정
	////pCamera->GenerateOrthographicProjectionMatrix(1.01f, 50.0f, FRAMEBUFFER_WIDTH, FRAMEBUFFER_HEIGHT);			// 직각 투영..?
	////
...
{
	m_GameTimer.Tick(60.0f);											// 시간 프레임 진행

	ProcessInput();

	AnimateObjects();

	HeapInitAndRendering();

	WaitForGpuComplete();						// GPU 대기하여 CPU와 동기화

	m_pdxgiSwapChain->Present(0, 0);			// 전후면 버퍼 플립

	MoveToNextFrame();							// 다음 프레임..
...
void CGameFramework::ProcessInput()				// Player의 연산 수행
{
	...
	{
		SetCursor(NULL);
		POINT ptCursorPos;						// 윈도우 귀속 시 마우스 좌표를 불러들일 때 매프레임마다 값의 변화를 반환하지 않고 특정 주기마다 반환
		GetCursorPos(&ptCursorPos);				// 여기서 특정 주기마다 커서가 이동하지만 SetCapture했을 때 커서 위치로 다시 돌아온다.
...
void CGameFramework::HeapInitAndRendering()
{
	HRESULT hResult = m_pd3dCommandAllocator->Reset();					// 명령 코드 할당 초기화
	hResult = m_pd3dCommandList->Reset(m_pd3dCommandAllocator, NULL);	// 명령 큐 초기화
...
void CGameFramework::ReleaseObjects()
{
	...
	if (m_pScene) m_pScene->ReleaseObjects();	// Scene에서 관리하는 오브젝트 해제
	
	if (m_pScene) delete m_pScene;			// Scene 해제

	if (m_pPlayer) delete m_pPlayer;			// Player 해제
...
{
	WaitForGpuComplete();		// GPU가 모든 명령 리스트를 실행할 때 까지 기다림

	ReleaseObjects();			// 오브젝트 해제
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	//std::cout << "CallBack (Keyboard)" << std::endl;
